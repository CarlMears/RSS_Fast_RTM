...
class AtmosProfile():
    ...
    def __init__(self,num_layers = 30,name = 'Default'):
        ...
        self.allowed_surface_types = ['Land','Ice','Ocean','Mixed']
    ...
    def estimate_vapor(self,surface_RH = 0.8,scale_height = 1500.0):
        ...
        if ('Z' in self.data and 'TS' in self.data and 'PS' in self.data):
    ...
    def calc_emissivity_surface_type_weighted(self,SurfEmiss,fov=0,surface_wts = [1.0,0.0,0.0]):  
        
        emiss_arr = np.zeros((3),dtype=np.float32)
        self.surface_type ='Mixed'
        
        emiss_arr[0] = float(SurfEmiss.OceanEmissivityFast(fov,
                                                           self.data['WS'],
                                                           self.data['TS']))

        emiss_arr[1] = float(SurfEmiss.SeaIceEmissivityFast(fov,self.data['TS']))
        emiss_arr[2] = 0.9
        emiss = np.sum(surface_wts*emiss_arr)
        self.addsurfacedatum(emiss,name = 'EM')
